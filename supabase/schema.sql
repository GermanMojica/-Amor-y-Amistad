...
-- 1. Limpieza de tablas previas (para asegurar la estructura correcta)
DROP TABLE IF EXISTS draw_assignments CASCADE;
DROP TABLE IF EXISTS participants CASCADE;
DROP TABLE IF EXISTS event_config CASCADE;

-- 2. Tabla de Configuración Global del Sorteo
...
-- 3. Tabla de Participantes (Identificados por Correo Electrónico o Usuario Único)
...
-- 4. Tabla de Asignaciones del Sorteo (Derangement)
...
CREATE INDEX idx_draw_assignments_giver ON draw_assignments(giver_id);
CREATE INDEX idx_draw_assignments_receiver ON draw_assignments(receiver_id);

-- 6. Insertar configuración inicial por defecto
